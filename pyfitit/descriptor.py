...
def getQuality_SVM(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
    ...
    if printDebug:
        print('Try predict by:', features)
    tryParams = [{'C': 1, 'gamma': 'scale'}, {'C': 10, 'gamma': 'auto'}]
    for il, label in enumerate(label_names):
        y = Y[:,il]
        classification = ML.isClassification(y)
        if mix: classification = False
        ...
        if model0 is None:
            ...
            for modelParams, j in zip(tryParams, range(len(tryParams))):
                if classification:
                    try_model[j] = SVC(kernel='linear', **modelParams)
                else:
                    try_model[j] = SVR(kernel='linear', **modelParams)
                if mix:
                    try_acc_mix, trueVals, pred, mod = mixture.score_cv(try_model[j], sample, features, label, label_names, makeMixtureParams, testRatio=0.5, repetitions=1, model_class=SVC(kernel='linear', **modelParams))
                    try_acc[j] = try_acc_mix['avgLabels']
                else:
                    try_acc[j] = np.mean(cross_val_score(try_model[j], X, y, cv=cv_count))
                    trueVals = y
            bestj = np.argmax(try_acc)
            acc[0] = try_acc[bestj]
            model = try_model[bestj]
            model_class = SVC(kernel='linear', **tryParams[bestj])
            if printDebug and len(tryParams)>1:
                print('Best model params: ', tryParams[bestj], f'Delta ={try_acc[bestj]-np.min(try_acc)}')
        ...
        for i in range(start_i, m):
            if model0 is not None: model = copy.deepcopy(model0)
            if mix:
                acc[i], trueVals, pred, mod = mixture.score_cv(model, sample, features, label, label_names, makeMixtureParams, testRatio=1 / cv_count, repetitions=cv_count, model_class=SVC(kernel='linear'))
            else:
                acc[i] = np.mean(cross_val_score(model, X, y, cv=cv_count))
...
def getXYFromSample_withfeaturescaling(sample, features, label_names, textColumn=None):
    """Construct X, y from sample with feature scaling
    ...
    if textColumn is None:
        ...
        return X_scaled, y, data.loc[:, textColumn].to_numpy()



def getQuality_SVM_linear(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
    ...
    predictedLabels = {}; trueLabels = {} 
    models = {}
    X,Y = getXYFromSample_withfeaturescaling(sample, features, label_names)
    if printDebug:
        print('Try predict by:', features)
    tryParams = [{'C': 1}, {'C': 10}]
    for il, label in enumerate(label_names):
        ...
        if model0 is None:
            ...
            for modelParams, j in zip(tryParams, range(len(tryParams))):
                if classification:
                    ...
                    try_model[j] = SVR(kernel='linear', **modelParams)
                if mix:
                    try_acc_mix, trueVals, pred, mod = mixture.score_cv(try_model[j], sample, features, label, label_names, makeMixtureParams, testRatio=0.5, repetitions=1, model_class=SVC(kernel='linear'))
                    try_acc[j] = try_acc_mix['avgLabels']
                else:
                    try_acc[j], pred = ML.score_cv(try_model[j], X, y, cv_count)
                    trueVals = y
...
def getQuality_SVM_rbf(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
    ...
    for il, label in enumerate(label_names):
        ...
        if model0 is None:
            ...
            for modelParams, j in zip(tryParams, range(len(tryParams))):
                if classification:
                    try_model[j] = SVC(kernel='rbf', **modelParams)
                else:
                    try_model[j] = SVR(kernel='rbf', **modelParams)
                if mix:
                    try_acc_mix, trueVals, pred, mod = mixture.score_cv(try_model[j], sample, features, label, label_names, makeMixtureParams, testRatio=0.5, repetitions=1, model_class=SVC(kernel='rbf'))
                    try_acc[j] = try_acc_mix['avgLabels']
                else:
                    try_acc[j], pred = ML.score_cv(try_model[j], X, y, cv_count)
                    trueVals = y
            ...
            acc[0] = try_acc[bestj]
            model = try_model[bestj]
            model_class = SVC(kernel='rbf', **tryParams[bestj])
            if printDebug and len(tryParams)>1:
                print('Best model params: ', tryParams[bestj], f'Delta ={try_acc[bestj]-np.min(try_acc)}')
...
def getQuality_SVM_poly(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
    ...
    for il, label in enumerate(label_names):
        ...
        if model0 is None:
            ...
            for modelParams, j in zip(tryParams, range(len(tryParams))):
                if classification:
                    try_model[j] = SVC(kernel='poly', degree=3, **modelParams)
                else:
                    try_model[j] = SVR(kernel='poly', degree=3, **modelParams)
                if mix:
                    try_acc_mix, trueVals, pred, mod = mixture.score_cv(try_model[j], sample, features, label, label_names, makeMixtureParams, testRatio=0.5, repetitions=1, model_class=SVC(kernel='poly', degree=3))
                    try_acc[j] = try_acc_mix['avgLabels']
                else:
                    try_acc[j], pred = ML.score_cv(try_model[j], X, y, cv_count)
                    trueVals = y
            ...
            acc[0] = try_acc[bestj]
            model = try_model[bestj]
            model_class = SVC(kernel='poly', degree=3, **tryParams[bestj])
            if printDebug and len(tryParams)>1:
                print('Best model params: ', tryParams[bestj], f'Delta ={try_acc[bestj]-np.min(try_acc)}')
...
def plot_descriptors_2d_SVMl(data, descriptor_names, label_names, labelMaps=None, folder_prefix='', unknown=None, markersize=500, textsize=8, alpha=1, cv_count=2, plot_only='', doNotPlotRemoteCount=0, textColumn=None, additionalMapPlotFunc=None):
    """Plot 2d prediction map.
    ...
    os.makedirs(folder2, exist_ok=True)
    if plot_only != 'data':
        qualityRes = getQuality_SVM_linear(data, descriptor_names, label_names, m=1, cv_count=cv_count, returnModels=True)
...
def plot_descriptors_2d_SVM_rbf(data, descriptor_names, label_names, labelMaps=None, folder_prefix='', unknown=None, markersize=500, textsize=8, alpha=1, cv_count=2, plot_only='', doNotPlotRemoteCount=0, textColumn=None, additionalMapPlotFunc=None):
    """Plot 2d prediction map.
    ...
    os.makedirs(folder2, exist_ok=True)
    if plot_only != 'data':
        qualityRes = getQuality_SVM_rbf(data, descriptor_names, label_names, m=1, cv_count=cv_count, returnModels=True)
...
def getQuality_ANN(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    mix = makeMixtureParams is not None
    quality = {}
    quality_std = {}
    predictedLabels = {}
    ...
    for il, label in enumerate(label_names):
        ...
        for i in range(m):
            ...
            if mix:
                acc[i], _, pred, mod = mixture.score_cv_ANN(model, sample, features, label, label_names, makeMixtureParams, testRatio=1 / cv_count, repetitions=cv_count, model_class=model_class)
            else:
                acc[i], pred = ML.score_cv_ANN(model, X_test, y_test, cv_count)
                trueVals = y_test
...
def getQuality_ANN2(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    mix = makeMixtureParams is not None
    quality = {}
    quality_std = {}
    predictedLabels = {}
    ...
    for il, label in enumerate(label_names):
        ...
        for i in range(m):
            ...
            if mix:
                # Use your custom cross-validation function for Keras models
                acc[i], pred = score_cv_keras(model, X_test, y_test, cv_count)
                trueVals = y_test
            else:
                # Use your custom cross-validation function for Keras models
                acc[i], pred = score_cv_keras(model, X_test, y_test, cv_count)
...
# Define a custom cross-validation function for Keras models
def score_cv_keras(model, X, y, cv_count):
    scores = []
    preds = []

    for train_index, test_index in KFold(n_splits=cv_count).split(X):
        X_train, X_test = X[train_index], X[test_index]
        y_train, y_test = y[train_index], y[test_index]
    ...
    return np.mean(scores), np.concatenate(preds)
...
def getQuality_ANN3(sample, features, label_names, makeMixtureParams=None, m=1, cv_count=10, printDebug=True):
    quality = {}
    quality_std = {}
    predictedLabels = {}
    trueLabels = {}
    ...
    for il, label in enumerate(label_names):
        ...
        for i in range(m):
            ...
            if classification:
                acc.append(accuracy_score(y_test, np.round(y_pred)))
            else:
                acc.append(r2_score(y_test, y_pred))

            if printDebug:
                print(f'Iteration {i+1}: {label} - Accuracy: {acc[-1]:.4f}')

        quality[label] = np.mean(acc)
        quality_std[label] = np.std(acc)
        predictedLabels[label] = y_pred
        trueLabels[label] = y_test
        models[label] = model

    if printDebug:
        print('')

    result = {label: {'quality': quality[label], 'quality_std': quality_std[label],
                      'predictedLabels': predictedLabels[label], 'trueLabels': trueLabels[label],
                      'model': models[label]} for label in label_names}
...
def getQuality_ANN4(sample, features, label_names, makeMixtureParams=None, m=1, cv_count=10, printDebug=True):
    quality = {}
    quality_std = {}
    predictedLabels = {}
    trueLabels = {}
    ...
    for il, label in enumerate(label_names):
        ...
        # classifier.add(Dense(output_dim = 128, init = 'uniform', activation = 'relu', input_shape=(X_train.shape[1],)))
